...
const chooseParticipantNum = ref(db, "posteriors/completedParticipants");
const runBTN = $("run");
var completionArray;
var trials;
...
let checkDefined = function(){
    console.log("BEGIN CHECK DEFINED--Completion array is: ", completionArray)
...
let selectParticipantNumber = function(){
    ...
    let openSlot = "posteriors/participants/participant" + parNum.slice(1);

    //find trials for participant
    participantNumRef = ref(db, openSlot); //issues???
...
let runSurvey = function(){        
    ...
    for (var i = 0; i <3; i++){
        ...
        results.push({"stim1":trials[trialNum]["stim1"], "stim2":trials[trialNum]["stim2"], "rating":null, "rt":null});
    ...
    question.innerHTML = ('<p>Given a person is engaging in the action:<br> <b>' + results[curq-1]["stim1"] + '</b> <br><br>how likely was it caused by the mental state:<br> <b>' + results[curq-1]["stim2"] + '</b>?</p>')
    let submit = $("submit");
    ...
    submit.addEventListener("click", function(e){
        if (moved){
            ...
            } else {                    //confused on how 
                prog = Math.round(100*(curq+1)/qn);
                progBar.style.width = prog + "%";
                progBar.innerText = (curq+1) + "/" + qn;
                question.innerHTML = ('<p>Given a person is engaging in the action:<br> <b>' + results[curq]["stim1"] + '</b> <br><br>how likely was it caused by the mental state:<br> <b>' + results[curq]["stim2"] + '</b>?</p>');
                d = new Date();
                start = d.getTime();
                curq = ++curq;
            }
